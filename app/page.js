    ...
    <div className="min-h-screen bg-white text-gray-800">
      ...
      <section id="pricing" className="py-20 px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { name: 'Free', price: '$0', period: 'forever', posts: '3 posts/month', features: ['All platforms', '6 languages', 'SEO optimization', 'Content history'], cta: 'Get Started Free', href: '/auth?mode=signup', style: 'border-gray-200' },
              { name: 'Pro', price: '$9', period: 'per month', posts: '50 posts/month', popular: true, features: ['All platforms', '6 languages', 'Full SEO tools', 'Priority generation', 'Email support'], cta: 'Start Pro Plan', href: '/upgrade', style: 'border-[#1B5FA8]' },
              { name: 'Premium', price: '$29', period: 'per month', posts: '300 posts/month', features: ['All platforms', '6 languages', 'Full SEO tools', 'Priority generation', 'Priority support', 'API access'], cta: 'Start Premium', href: '/upgrade', style: 'border-[#0D9488]/50' },
            ].map(p => (
              <div key={p.name} className={`rounded-xl p-6 border-2 relative shadow-sm bg-white ${p.style} ${p.popular ? 'shadow-[#1B5FA8]/10' : ''}`}>
                {p.popular && (
                ...
                <Link href={p.href} className={`block text-center py-3 rounded-lg font-semibold text-sm transition-colors ${p.popular ? 'bg-[#1B5FA8] hover:bg-[#1B5FA8]/90 text-white' : 'border-2 border-gray-300 hover:border-[#1B5FA8] hover:text-[#1B5FA8] text-gray-600'}`}>
                  {p.cta}
                </Link>
              </div>
            ))}
      ...
      <section className="py-20 px-6 text-center bg-gradient-to-br from-[#1B5FA8]/5 to-[#0D9488]/5">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-gray-500 mb-8 text-lg">Join marketers and business owners using RANKIVO to grow faster. Start free today.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/auth?mode=signup" className="w-full sm:w-auto bg-[#0D9488] hover:bg-[#0D9488]/90 text-white px-8 py-4 rounded-xl font-bold text-lg transition-colors shadow-lg shadow-[#0D9488]/20 text-center">
              Start Free Today
            </Link>
            <Link href="/upgrade" className="w-full sm:w-auto border-2 border-[#C9943A] hover:bg-[#C9943A]/10 text-[#C9943A] px-8 py-4 rounded-xl font-bold text-lg transition-colors text-center">
              View All Plans
            </Link>
          </div>
        </div>
