...
const SEO_TOOLS = [
  ...
  { href: '/tools/instagram-caption-generator', label: 'Instagram Captions',       icon: '📸' },
  { href: '/tools/tiktok-caption-generator',    label: 'TikTok Captions',          icon: '🎵' },
  { href: '/tools/linkedin-post-generator',     label: 'LinkedIn Posts',           icon: '💼' },
  { href: '/tools/x-post-generator',            label: 'X Posts',                  icon: '✖️' },
  { href: '/tools/email-generator',             label: 'Email Generator',          icon: '📧' },
  { href: '/tools/youtube-script-generator',    label: 'YouTube Script',           icon: '🎬' },
  { href: '/tools/ad-copy-generator',           label: 'Ad Copy Generator',        icon: '📣' },
]
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur border-b border-gray-200 shadow-sm">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {toolsOpen && (
              <div className="absolute top-full left-0 mt-2 w-60 bg-white border border-gray-200 rounded-xl shadow-lg py-1.5 z-50 max-h-[80vh] overflow-y-auto">
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wider px-4 pt-2 pb-1">SEO & Research</p>
                {SEO_TOOLS.slice(0, 4).map(t => (
                  <Link key={t.href} href={t.href} onClick={() => setToolsOpen(false)}
                    className="flex items-center gap-2.5 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 hover:text-[#1B5FA8] transition-colors">
                    <span>{t.icon}</span>{t.label}
                  </Link>
                ))}
                <div className="border-t border-gray-100 my-1" />
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wider px-4 pt-2 pb-1">Content Generators</p>
                {SEO_TOOLS.slice(4).map(t => (
                  <Link key={t.href} href={t.href} onClick={() => setToolsOpen(false)}
                    className="flex items-center gap-2.5 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 hover:text-[#1B5FA8] transition-colors">
                    <span>{t.icon}</span>{t.label}
                  </Link>
          ...
          {staticLinks.map(l => (
            <Link key={l.href} href={l.href} className="text-gray-500 hover:text-[#1B5FA8] text-sm transition-colors">
              {l.label}
            </Link>
          ))}
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-gray-200 px-6 py-4 space-y-3">
          <div>
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">SEO & Research</p>
            {SEO_TOOLS.slice(0, 4).map(t => (
            ...
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mt-3 mb-1">Content Generators</p>
            {SEO_TOOLS.slice(4).map(t => (
              <Link key={t.href} href={t.href} onClick={() => setOpen(false)}
                className="flex items-center gap-2 text-gray-600 hover:text-[#1B5FA8] text-sm py-1.5">
                <span>{t.icon}</span>{t.label}
          ...
          <div className="border-t border-gray-100 pt-3 space-y-1.5">
            {staticLinks.map(l => (
              <Link key={l.href} href={l.href} onClick={() => setOpen(false)}
                className="block text-gray-600 hover:text-[#1B5FA8] text-sm py-1">{l.label}</Link>
            ))}
          </div>
